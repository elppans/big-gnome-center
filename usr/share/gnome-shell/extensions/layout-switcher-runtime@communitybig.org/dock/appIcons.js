...
const COMMUNITY_INDICATOR_SCHEMA = 'org.communitybig.panel-and-dock';
const COMMUNITY_INDICATOR_CLASSES = new Map([
    ['dot', 'community-indicator-dot'],
    ['hybrid', 'community-indicator-hybrid'],
    ['desk-ux', 'community-indicator-desk-ux'],
]);
const COMMUNITY_INDICATOR_GEOMETRY = new Map([
...
export const DockAbstractAppIcon = GObject.registerClass({
    ...
    _updateFocusState() {
        ...
        const ownsFocusedWindow = focusWindow &&
            (this.ownsWindow(focusWindow) || this.getWindows().includes(focusWindow));
        this.focused = this.running &&
            (tracker.focus_app === this.app || ownsFocusedWindow);
    }
    ...
    _syncCommunityIndicatorStyle() {
        ...
        this.add_style_class_name(
            COMMUNITY_INDICATOR_CLASSES.get(configured) ??
            COMMUNITY_INDICATOR_CLASSES.get('dot'));
        this._syncCommunityIndicatorAppearance();
    }
    ...
    _syncCommunityIndicatorAppearance() {
        ...
        this._dot.x_align = position === St.Side.LEFT
            ? Clutter.ActorAlign.START
            : position === St.Side.RIGHT
                ? Clutter.ActorAlign.END
                : Clutter.ActorAlign.CENTER;
        this._dot.y_align = position === St.Side.TOP
            ? Clutter.ActorAlign.START
        ...
        const color = this.focused
            ? '-st-accent-color'
            : 'rgba(160, 160, 168, 0.72)';
        this._dot.set_size(width, height);
        this._dot.set_style(
    ...
    _updateDotStyle() {
        ...
        if (this._communityIndicatorSettings)
            this._syncCommunityIndicatorAppearance();
    }
...
, class DockShowAppsIcon extends Dash.ShowAppsIcon {
    ...
    _hasPopupMenu() {
        // Community Dock is configured exclusively by Big Gnome Center.
        return false;
    }
